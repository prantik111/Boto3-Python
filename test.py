...
all_regions = []
year,month,date,acc_id,prod_code,box_usage,az,res_id,res_type,res_region,total_up_time,all_regions,num_instances,cluster_ins_type ,cluster_instances_running_count,emr_box_usage,emr_box_usage_list,emr_box_usage_list,ec2_instance_list= ([] for i in range(19))
...
for each_region in all_regions:
    ...
    if len(emr_response["Clusters"]) != 0:
        for each_cluster in emr_response["Clusters"]:
            ...
            res_region.append(each_region)
            num_instances.append(sum(cluster_instances_running_count))
