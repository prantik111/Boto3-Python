...
def Cluster_Instance_Fleet_Details(cluster_id):
    ...
    print(cluster_id)
    print("************* testing")
   # print(instance_fleet_response)
    cluster_instances_running_count = instance_fleet_response
    temp1=[]
    for each in instance_fleet_response["InstanceFleets"]:
        ...
        if each["Name"].startswith("Core"):
            temp1.append(each["TargetSpotCapacity"])
        elif each["Name"].startswith("Master"):
            temp1.append(each["TargetOnDemandCapacity"])
        else:
            print("do nothing")
...
for each_region in all_regions:
    ...
    if len(emr_response["Clusters"]) != 0:
        for each_cluster in emr_response["Clusters"]:
            ...
            Cluster_List_Instance_Details(cluster_id)
